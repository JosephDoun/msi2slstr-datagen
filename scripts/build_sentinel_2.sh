...
read -d '\n' -a AZIMUTHS < <(cat $s2_dir/GRANULE/*/MTD_TL.xml | grep -oP "(?<=<AZIMUTH_ANGLE unit=\"deg\">)[0-9\.]*(?=</AZIMUTH_ANGLE>)");
...
log "Building VRT file for merged Sentinel-2 scene."
...
log "Building multiband TIF file for Sentinel-2 scene."
